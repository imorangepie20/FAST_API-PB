"""
M1 Predictor — Divergence-Based Routing용 M1 Ridge Regression 래퍼

M1의 AudioFeaturePredictor를 로드하여
DBR에서 QLTY 예측과 비교하는 데 사용한다.
...
def _get_predictor():
    ...
    if not model_path.exists():
        logger.warning(f"[DBR M1] M1 모델 파일 없음: {model_path}")
        return None
    ...
    try:
        ...
        logger.info("[DBR M1] M1 AudioFeaturePredictor 로드 완료")
        return _predictor
    except Exception as e:
        logger.error(f"[DBR M1] M1 모델 로드 실패: {e}")
        return None
...
def predict(
    ...
    try:
        ...
        logger.error(f"[DBR M1] 예측 실패: {e}")
        return None


def is_loaded() -> bool:
    """M1 모델 로드 여부 확인."""
    return _predictor is not None or _get_predictor() is not None
